...
if os.path.exists(os.path.join(parse_args.save_path,"sgdm_drift")):
    shutil.rmtree(os.path.join(parse_args.save_path,"sgdm_drift"))


os.makedirs(os.path.join(parse_args.save_path,"sgdm_drift"),exist_ok=True)
...
for ckpt in tqdm(list(os.listdir(os.path.join(args.save_path,parse_args.load_from_mg_spacing,"states")))):
    ...
    for i,(img,lbl) in zip(range(NUM_STEPS),ds_train):
        ...
        if grads == None:
            grads = {key : val["kernel"] for key,val in grad.items() if  substrings_in_path(key.lower(),"|".join(["Conv_0","Conv_1","Conv_2","Conv_3","Conv_4","Conv_5","Conv_6","Conv_7"]))}
        else:
            grads = tree_map(lambda g1,g2 : g1+g2,grads,{key : val["kernel"] for key,val in grad.items() if  substrings_in_path(key.lower(),"|".join(["Conv_0","Conv_1","Conv_2","Conv_3","Conv_4","Conv_5","Conv_6","Conv_7"]))})
    
    grads_mean = tree_map(lambda x : x/NUM_STEPS,grads)

    trace = optimizer_state[1][0].trace

    trace = {key : val["kernel"] for key,val in trace.items() if  substrings_in_path(key.lower(),"|".join(["Conv_0","Conv_1","Conv_2","Conv_3","Conv_4","Conv_5","Conv_6","Conv_7"]))}
    ...
    #print(optimizer_state[1][0])
    for key in grads.keys():
        measure[key] = {"trace" : {"cos" : tree_map(lambda t1,t2 : cosine_sim_fn(t1,t2),trace[key],grads_mean[key]),
                                   "dist" :  tree_map(lambda t1,t2 : dist_fn(t1,t2),trace[key],grads_mean[key])}}
    ...
    with open(os.path.join(args.save_path,"sgdm_drift",ckpt), "wb") as f:
        pkl.dump(measure,f)