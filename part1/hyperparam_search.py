...
for norm_fn in ["global_center_std_uncenter"]:
...
for norm_fn in ["global_center_std_uncenter"]:
    for norm_scale in [0.05 + 0.05*x for x in range(10)]:
        for norm_every in [1]:
            try:
                os.makedirs("./exps_sgdm/global_center_std_uncenter/" + str(norm_scale) + "_" + str(norm_every),exist_ok=False)
            except:
                continue
            default["optimizer"]["lambda_wd"] = 0
            default["norm"]["change_scale"] = "identity"
            ...
            default["norm"]["norm_multiply"] = norm_scale
            default["norm"]["norm_every"] = norm_every
            with open("./exps_sgdm/global_center_std_uncenter/" + str(norm_scale) + "_" + str(norm_every) + "/settings.json","w") as f:
                json.dump(default,f)

            print("python main.py " + "./exps_sgdm/global_center_std_uncenter/" + str(norm_scale) + "_" + str(norm_every) + "/")
...
exps_1 = []
exps_2 = []
for wd in [0.015+0.001*x for x in range(6)]:
    os.makedirs("./exps_sgdm/" + "wd" + "/" + str(wd))
    default["optimizer"]["lambda_wd"] = wd
    with open("./exps_sgdm/" + "wd" + "/" + str(wd) + "/settings.json","w") as f:
        json.dump(default,f)

    exps_1.append("python main.py " + "./exps_sgdm/" + "wd" + "/" + str(wd) + "/")
...
for wd in [0.015+0.001*x for x in range(6)]:
    ...
    exps_2.append("python main.py " + "./exps_sgdm/" + "wd_slim" + "/" + str(wd) + "/")

for e1,e2 in zip(exps_1,exps_2):
    print(e1)
    print(e2)
"""